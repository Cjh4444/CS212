 ...
 * 05/25/2024
 * Program 9
 * Set + Iterator Data Structure (with ability for iterator to handle added data)
 */
...
public:
    ...
    public:
        ...
        BSTIterator(const BSTIterator& copyMe) {
            startingNode = copyMe.startingNode;
            currentNode = copyMe.currentNode;
            reachedEnd = copyMe.reachedEnd;
            iteratedSet = copyMe.iteratedSet;
        }
        ...
        BSTIterator& operator++() {
            if (!reachedEnd) {
                if (currentNode) {
                    iteratedSet->Add(currentNode->data);
                ...
                if(!currentNode) {
                    ...
                    while(currentNode) {
                        if (!iteratedSet->Contains(currentNode->data)) {
                            iteratedSet->Add(currentNode->data);
                            break;
                        }
                        currentNode = currentNode->GetSuccessor();
